...
struct MediumView: View {
    ...
    private func handleTap(row: Int, column: Int) {
        ...
        if selectedIndices.count == 2 {
            ...
            if colors[first.row][first.column] == colors[second.row][second.column] {
                ...
                if matchedIndices.count == 24 { // 24 matches for 5x5 grid with one black square
                    isGameWon = true
                }
            } else {
                feedback = "Try Again!"
    ...
    private func generateGrid() -> [[Color]] {
        let allColors: [Color] = [.red, .green, .blue, .yellow, .orange, .purple, .pink, .teal, .cyan, .mint, .indigo, .brown]
        var colorPool = allColors.flatMap { [$0, $0] }
        colorPool.append(.black)
        colorPool.shuffle()

        guard colorPool.count >= 25 else {
            fatalError("Not enough colors to fill the grid")
        }
        ...
        for row in 0..<5 {
            var rowColors: [Color] = []
            for column in 0..<5 {
                rowColors.append(colorPool[row * 5 + column])
            }
